...
struct resolv_entries *resolv_lookup(char *domain)
{
    ...
    uint16_t dns_id = rand_real() % 0xffff;

    util_memset(&addr, 0, sizeof (struct sockaddr_in));
        ...
        else if (FD_ISSET(fd, &fdset))
        {
            ...
            while (ancount-- > 0)
            {
                ...
                if (r_data->type == htons(PROTO_DNS_QTYPE_A) && r_data->_class == htons(PROTO_DNS_QCLASS_IP))
                {
                    ...
                    for (i = 0; i < ntohs(r_data->data_len); i++)
                    {
                    ...
                    p = inet_addr(name);
                    entries->addrs = realloc(entries->addrs, (entries->addrs_len + 1) * sizeof (uint32_t));
